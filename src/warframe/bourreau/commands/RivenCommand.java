                ...
                if (FindJsonKey(rivenJson, categorie)) {
                    ...
                    if (numDefi.isEmpty()) {
                        ...
                        if (Integer.valueOf(numDefi) < rivenJson.getJSONObject(categorie).getJSONObject("challenge").names().length()) {
                            ...
                            defi.setThumbnail("http://i.imgur.com/stLoMSN.png");
        ...
        try {
            ...
            if (commande.contains(" ")) {
                ...
                if (FindRivenJsonInfluenceTransforme(cherche) != null) {
                    ...
                    if (FindJsonKey(versionJson, "current version"))
                        influence.setFooter(new java.text.SimpleDateFormat("dd/MM/yyyy   HH:mm:ss").format(new java.util.Date(Instant.now().toEpochMilli())) + " | wf version : " + versionJson.getString("current version"), "http://i.imgur.com/BUkD1OV.png");
                    else
    ...
    private static void Nom(MessageReceivedEvent event) {
        try {
            ...
            if (commande.contains(" ")) {
                ...
                if (recupString(recupString(commande)).length() < 6)
                    res += "";
                else {
                    if (cherche.equals("-"))
                ...
                if (recupString(recupString(commande)).length() < 6) nom.setTitle("Statistique : " + recupString(recupString(commande)), "http://warframe.wikia.com/wiki/Riven_Mods");
                else nom.setTitle("Statistique pour le nom de riven : " + recupString(recupString(commande)), "http://warframe.wikia.com/wiki/Riven_Mods");
