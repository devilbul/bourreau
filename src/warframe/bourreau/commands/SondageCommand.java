...
public class SondageCommand extends  Command {
    ...
    public static void Sondage(MessageReceivedEvent event) {
        if (event.getMessage().getContent().contains(" ")) {
            ...
            String adresseID = System.getProperty("user.dir") + File.separator + "sondage" + File.separator + "id.json";
            ...
            switch (commande) {
                ...
                case "reponses":
                    if (ReponsesSondage(event, adresseSondage, adresseVote))
                        AfficheSondage(event, adresseSondage);
                        //PinnedSondage(event, adresseSondage, adresseID);
                    break;
                case "clear":
                    ClearSondage(event, adresseSondage, adresseVote, adresseID);
                    break;
                case "create":
                    CreateSondage(event, adresseSondage, adresseVote, adresseID);
                    break;
                case "resultat":
        ...
        try {
            if (new File(adresseSondage).exists()) {
                ...
                if (sondageJson.toString().contains("reponses")) {
                    ...
                    sondageDisplay.setColor(new Color(17, 204, 17));
    ...
    private static void ClearSondage(MessageReceivedEvent event, String adresseSondage, String adresseVote, String adresseID) {
        try {
            if (new File(adresseSondage).exists() && new File(adresseVote).exists() && new File(adresseID).exists()) {
                String ID = new String(Files.readAllBytes(Paths.get("sondage" + File.separator + "id.json")));
                JSONObject IDJson = new JSONObject(ID);

                if (IDJson.toString().contains("idTC") && IDJson.toString().contains("idM")) {
                    String IDTextChannel = IDJson.getString("idTC");
                    String IDMessage = IDJson.getString("idM");

                    event.getJDA().getTextChannelById(IDTextChannel).getMessageById(IDMessage).complete().unpin();
                }

                new File(adresseSondage).delete();
                new File(adresseVote).delete();
                new File(adresseID).delete();
    ...
    private static void CreateSondage(MessageReceivedEvent event, String adresseSondage, String adresseVote, String adresseID) {
        try {
            if (!(new File(adresseSondage).exists())) {
                ...
                if (commande.contains(" ")) {
                    ...
                    FileWriter fileID = new FileWriter(adresseID);
    ...
    private static void PinnedSondage(MessageReceivedEvent event, String adresseSondage, String adresseID) {
        try {
            if (new File(adresseSondage).exists()) {
                FileWriter file = new FileWriter(adresseID);
                String sondage = new String(Files.readAllBytes(Paths.get("sondage" + File.separator + "sondage.json")));
                String IDTextChannel;
                String IDMessage;
                String reponses = "";
                EmbedBuilder sondageDisplay = new EmbedBuilder();
                JSONObject sondageJson = new JSONObject(sondage);
                ...
                IDTextChannel = event.getTextChannel().getId();
                IDMessage = event.getTextChannel().sendMessage(sondageDisplay.build()).complete().getId();
                event.getTextChannel().getMessageById(IDMessage).complete().pin();
                IDJson.put("idTC", IDTextChannel);
                IDJson.put("idM", IDMessage);

                file.write(IDJson.toString());
                file.flush();
                file.close();
            }
        }
        ...
        try {
            if (new File(adresseSondage).exists() && new File(adresseVote).exists()) {
                ...
                if (sondageJson.toString().contains("reponses")) {
                    ...
                    resultat.setColor(new Color(17, 204, 17));
                    ...
                    if (commande.contains(" ")) {
                        ...
                        if (!FindVotant(auteur, voteJson)) {
                            if (FindList(reponsesList, reponse)) {
                                ...
                                event.getTextChannel().sendMessage("vote enregistré.").queue();
