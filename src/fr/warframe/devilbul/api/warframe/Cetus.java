    ...
    public void checkCetusNight() {
        final Runnable beeper = () -> {
            ...
            try (InputStream is = new URL(urlApi).openStream()) {
                ...
                String idMessage = eidolonJson.getString("id_message");
                ...
                if (nextNight < 3000001 && nextNight > 59999) {
                    ...
                    if (idMessage.isEmpty()) {
                        ...
                        file.write(eidolonJson.put("id_message", message.getId()).toString(3));
                        file.flush();
                        file.close();
                    } else
                        Bourreau.jda.getGuildById(idServeur).getTextChannelById(idTextChannelEidolon).getMessageById(idMessage).complete().editMessage(embed.build()).queue();
                    ...
                    if (nextNight < (3060001 + timePing) && nextNight > (2999999 + timePing)) {
                        if (idTextChannelEidolon != null && idRoleEidolon != null && eidolonJson.getBoolean("ping")) {
                            mention = Bourreau.jda.getGuildById(idServeur).getTextChannelById(idTextChannelEidolon).sendMessage(
                                    Bourreau.jda.getGuildById(idServeur).getRoleById(idRoleEidolon).getAsMention() + " Eidolon dans " + parseEpochToTime(nextNight)
                            ).complete();

                            mention.delete().completeAfter(timePing + 3000000, MILLISECONDS);
                        }
                    }
